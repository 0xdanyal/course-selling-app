...
const {purchaseModel} = require("../schemas")
...
courseRouter.post("/purchase", authMiddleware, async function(req,res) {
    // Extract userId from the request object, which was set by the authMiddleware
    ...
    if(!courseId){
        return res.status(400).json({
            message: "Please provide a courseId OR choose a course",
        });
    }
